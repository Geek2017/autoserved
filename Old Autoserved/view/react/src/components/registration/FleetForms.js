...
const { registerFleet } = authActions;

class FleetForms extends Component {
  constructor(props) {
    super(props);
    this.state = {
      countdown: 5,
  ...
  _handleFleetName = event => {
    this.setState({ txtFleetName: event.target.value });
  };
  ...
  _submit = event => {
    ...
    this.setState({ attemptRegister: true }, () => {
      ...
      let successMessages = [];
      let infoMessages = [];
      const { registerFleet, t, history } = this.props;
      const {
        selCountry: country,
        txtFirstName: fname,
        txtLastName: lname,
        ...
        .then(fleet => {
          if (fleet) {
            successMessages.push({
              message: t("translation.successRegisterFleet", fleet)
            });
            this.setState(
              {
                attemptRegister: false,
        ...
        .catch(error => {
          ...
          if (response) {
            ...
            if (data.errors) {
              ...
              errorMessages.push({ message: data.message });
            }
          } else if (data) {
            const { errors } = data;
  ...
  render = () => {
    ...
    const validFleetName = validator.isValidCompanyName(txtFleetName);
        ...
        <Form>
          <FormGroup>
            ...
            <FormInput
              ...
              onChange={this._handleFleetName}
              value={txtFleetName || ""}
              invalid={txtFleetName !== null && !validFleetName}
            />
            <FormFeedback valid={validFleetName}>
              Invalid fleet name
            </FormFeedback>
          </FormGroup>
          <Row>
...
export default withRouter(
  withTranslation()(connect(mapStateToProps, { registerFleet })(FleetForms))
);
