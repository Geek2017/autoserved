angular.module('newApp').controller('jobordersCrtl', function($scope, $http, $filter, $timeout, $window) {
                ...
                db.ref('/joborders/')
                    ...
                    .then(function(snapshot) {
                        snapshot.forEach(function(childSnapshot) {
                            childSnapshot.ref.child('state').set(4);
                        });
                    });
    ...
    $scope.repairorder = function() {
        ...
        firebase.database().ref('/estimate/').orderByChild('ekey').equalTo(itemkey).on("value", function(snapshot) {
            let returnArr = [];
            $timeout(function() {
                $scope.$apply(function() {
                    snapshot.forEach(childSnapshot => {
    ...
    $scope.completejo = function() {
        ...
        db.ref('/joborders/')
            ...
            .then(function(snapshot) {
                snapshot.forEach(function(childSnapshot) {
                    childSnapshot.ref.child('state').set(6);
                });
            });
