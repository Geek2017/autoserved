...
jQuery.fn.selectToUISlider = function(a) {
    function b(a) { return a.constructor == Array }

    function c(a) { return "text" == f.tooltipSrc ? h[a].text : h[a].value }

    function d(a) { return (a / (h.length - 1) * 100).toFixed(2) + "%" }
    var e = jQuery(this),
        f = jQuery.extend({ "labels": 3, "tooltip": !0, "tooltipSrc": "text", "labelSrc": "value", "sliderOptions": null }, a),
        g = function() { var a = []; return e.each(function() { a.push("handle_" + jQuery(this).attr("id")) }), a }(),
        h = function() { var a = []; return e.eq(0).find("option").each(function() { a.push({ "value": jQuery(this).attr("value"), "text": jQuery(this).text() }) }), a }(),
        i = function() { if (e.eq(0).find("optgroup").size() > 0) { var a = []; return e.eq(0).find("optgroup").each(function(b) { a[b] = {}, a[b].label = jQuery(this).attr("label"), a[b].options = [], jQuery(this).find("option").each(function() { a[b].options.push({ "text": jQuery(this).text(), "value": jQuery(this).attr("value") }) }) }), a } return null }(),
        j = {
            "step": 1,
            "min": 0,
            ...
            "values": function() { var a = []; return e.each(function() { a.push(jQuery(this).get(0).selectedIndex) }), a }()
        };
    ...
    k.find(".ui-slider-scale dt").each(function(a) { jQuery(this).css({ "left": (100 / i.length * a).toFixed(2) + "%" }) }), k.insertAfter(jQuery(this).eq(this.length - 1)).slider(f.sliderOptions).attr("role", "application").find(".ui-slider-label").each(function() { jQuery(this).css("marginLeft", -jQuery(this).width() / 2) }), k.find(".ui-tooltip-pointer-down-inner").each(function() {
    ...
    return b(p) ? jQuery(p).each(function(a) { k.find(".ui-slider-tooltip .ttContent").eq(a).text(c(this)) }) : k.find(".ui-slider-tooltip .ttContent").eq(0).text(c(p)), this
};